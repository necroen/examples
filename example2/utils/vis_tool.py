...
def vis_bbox(img, bbox, label=None, score=None, ax=None):
    ...
    Args:
        img (np): (3, height, width) RGB format  range [0, 255]
        bbox (np): (R, 4)   ymin xmin ymax xmax
        label (np): (R,)
            The values correspond to id for label names stored in
            label_names. This is optional.
        score (np): float array (R,)
    ...
    for i, bb in enumerate(bbox):  # bbox   ymin xmin ymax xmax
        xy = (bb[1], bb[0])    # (xmin, ymin)
        height = bb[2] - bb[0]  # ymax - ymin = h
        width = bb[3] - bb[1]   # xmax - xmin = w
